    ...
    async def execute_stream(
        ...
        *,
        ...
        system_msg = (
            ...
            " For cyclical baskets, use query_universe with filters={'theme':'cyclical'} "
            "instead of hand-writing symbols unless the tool returns BLOCKED. If "
            "query_fundamentals_pit returns NO_DATA/PARTIAL_COVERAGE or INVALID_REQUEST "
            "because the date window is too large, call run_fundamental_data_update with "
            "dry_run=true and auto_chunk=true, then when live update is allowed call it "
            "again with dry_run=false, auto_chunk=true, execute_plan=true and verify with "
            "query_fundamentals_pit. If query_macro_series_pit returns NO_DATA or "
            "INVALID_REQUEST, use known_datasets and run_macro_data_update with "
            "dry_run=true, auto_chunk=true; when live update is allowed execute with "
            "dry_run=false, auto_chunk=true, execute_plan=true and verify with "
            "query_macro_series_pit. Do not infer macro timing signals from missing or "
            "unknown macro datasets. If "
            "generate_factor_code returns UNSUPPORTED_FORMULA, or "
            "run_factor_static_checks returns semantic_status=FAILED, do not save, "
            "evaluate, rank, or recommend that generated factor. Treat "
            ...
            "diagnostics fail or predictive evidence is not computed; use failed "
            "candidate, blocked candidate, or needs repair instead of "
            ...
            "dry_run=false; if the tool returns BLOCKED/INVALID_REQUEST/NO_DATA, "
