...
| Order plans | `services/order_plan_service.py:save_order_plan`, `append_order_plan_event`; CLI injects `PersistencePaths.order_plans_root` | `{order_plans_root}/{order_plan_id}.json`, manifest, and `.events/{sha256(order_plan_id)}.jsonl` | Immutable JSON plan plus schema-versioned append-only lifecycle events | Existing valid plans are reread, schema/id validated, hashed, and manifested under one artifact-root lock; callers parse the returned locked bytes. Event append/read shares the event lock and tolerates only an incomplete final tail. | Reused ids, concurrent legacy changes, invalid files, tampered bytes, unknown event schema versions, and malformed complete records fail closed. Events never mutate the plan. |
